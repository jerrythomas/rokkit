<script>
	...
	const contentData = {
		Dashboard: {
			...
			description:
				'Welcome to your main dashboard. Here you can see key metrics and recent activity.',
...
<div class="h-64 overflow-hidden rounded-lg border border-gray-200">
	<ContentNavigator vertical>
		...
		{#snippet content()}
			<div class="flex-grow p-6">
				<div>
					<h3 class="mb-2 text-lg font-semibold">{currentContent.title}</h3>
					<p class="mb-4 text-gray-600">{currentContent.description}</p>
					<div class="space-y-2">
						{#each currentContent.metrics as metric}
							<div class="rounded bg-blue-50 p-2 text-sm">{metric}</div>
						{/each}
					</div>
				</div>
			</div>
