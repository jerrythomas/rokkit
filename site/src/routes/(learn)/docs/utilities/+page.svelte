...
<article data-article-root>
	...
	<p>
		...
		(pure state machines with no DOM dependency) and the <strong>navigator</strong> Svelte action
		(which binds a controller to the DOM, adds keyboard handlers, and manages ARIA attributes).
	</p>
	<p>
		The separation means you can use a controller in tests without a browser, and swap the
		visual rendering without touching the interaction logic. It also means you can build your
		own accessible components by reusing the same controllers Rokkit uses internally.
	</p>

	<h2>State Management</h2>
	<p class="text-[1.0625rem] leading-7 text-surface-z6 mb-8">
		Rokkit components use Svelte 5's <code>$state</code> and <code>$bindable</code> runes for
	...
	<p>
		...
		corresponds to the <code>value</code> field from your data (or the item itself for
		primitives):
	</p>
	...
	<h3>Change Events</h3>
	<p>
		Use <code>onchange</code> or <code>onselect</code> for side effects when selection
	...
	<ul>
		<li><strong>Uncontrolled</strong> — Omit <code>bind:value</code>. The component manages its own state internally. Use <code>onchange</code> to react to changes.</li>
		<li><strong>Controlled</strong> — Use <code>bind:value</code> to sync state with your application. The component and your state stay in sync automatically.</li>
	</ul>
	...
	<p>
		<code>Tabs</code> and <code>Toggle</code> also use <code>bind:value</code> with the
		same pattern:
	</p>
	...
	<p>
		...
		intercepts keyboard events, routes them through the controller, and syncs the DOM
		focus and ARIA state in response.
	</p>
	<h3>Usage</h3>
	...
	<p>
		...
		focused, what is selected, whether groups are expanded. They have no DOM dependency and
		can be tested without a browser.
	</p>
	<h3>Available Controllers</h3>
	<ul>
		<li><strong>ListController</strong> — flat and grouped list navigation with single/multi-select</li>
	...
	<p>
		...
		Components reference icons by semantic name (e.g., <code>expand</code>, <code>check</code>),
	...
	<table>
		...
		<tbody>
			...
			<tr><td><code>collapse</code></td><td>Select, Tree, List</td><td>Collapse/close indicator</td></tr>
	...
	<p>
		...
		Pass an <code>icons</code> prop with the names you want to change:
	</p>
	<Code content={perComponent} language="svelte" />

	<h3>Item-Level Icons</h3>
	<p>
		Map an <code>icon</code> field in your data to display per-item icons. The icon value
	...
	<p>Full reference and interactive demo available on the <a href="/docs/utilities/connector">Connector page</a>.</p>
