...
<article data-article-root>
	<p class="text-surface-z6 mb-8 text-[1.0625rem] leading-7">
		<code>@rokkit/core</code> provides shared utilities, constants, and field mapping helpers used
		across all Rokkit packages. Most applications never import it directly — components consume it
		internally. Import directly when building custom components that integrate with Rokkit's
	...
	<table>
		...
		<tbody>
			...
			<tr><td><code>getValue(item)</code></td><td>Read the value field (falls back to raw item)</td></tr>
			<tr><td><code>getIcon(item)</code></td><td>Read icon field, handles state-based icon objects</td></tr>
			<tr><td><code>hasChildren(item)</code></td><td>True when item has a non-empty children array</td></tr>
...
// children, type, snippet, href, disabled, expanded, selected</code></pre>
	...
	<table>
		...
		<tbody>
			<tr><td><code>slugify(text)</code></td><td>Convert text to URL-safe slug (lowercase, hyphens)</td></tr>
			<tr><td><code>truncate(text, length)</code></td><td>Truncate with ellipsis at word boundary</td></tr>
	...
	<ul>
		<li>
			Building custom components that need to interpret the same <code>fields</code> prop as Rokkit
			components
		</li>
	...
	<p>
		For most feature work, rely on components and <code>@rokkit/states</code> to handle field
		mapping internally.
	</p>
	...
	<ul>
		...
		<li>
			<a href="/docs/data-binding/field-mapping">Field Mapping</a> — How the fields prop works in
			components
		</li>
	</ul>
</article>
