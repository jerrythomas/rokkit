<article data-article-root>
	<p>
		...
		<code>dataset()</code> pipeline for grouping, alignment, rollup aggregation, filling missing
		values, and set joins — plus standalone join functions and a declarative filter engine.
	</p>

	<h2>Import</h2>
	<pre><code>{`import { dataset, crossJoin, leftJoin, filterData } from '@rokkit/data'`}</code></pre>

	<h2>dataset() — fluent pipeline</h2>
	<p>
		<code>dataset(rows)</code> wraps an array of plain objects and returns a chainable
		<code>DataSet</code>. Call <code>.select()</code> at the end to execute the pipeline and get
		the result array back.
	</p>
	<pre><code>{`const result = dataset(rows)
  ...
  .select()`}</code></pre>
	...
	<table>
		...
		<tbody>
			...
			<tr><td><code>.fillNA(defaults)</code></td><td>Fill null/undefined values with defaults</td></tr>
			<tr><td><code>.update(value|fn)</code></td><td>Update rows matching the current <code>.where()</code></td></tr>
			<tr><td><code>.remove()</code></td><td>Delete rows matching the current <code>.where()</code></td></tr>
			<tr><td><code>.union(other)</code></td><td>Append rows from another dataset</td></tr>
			<tr><td><code>.minus(other)</code></td><td>Rows in this dataset not in <code>other</code></td></tr>
			...
			<tr><td><code>.alignBy(...keys)</code></td><td>Ensure all values of a key appear in every group</td></tr>
			<tr><td><code>.usingTemplate(obj)</code></td><td>Default field values for rows added by <code>alignBy</code></td></tr>
			<tr><td><code>.summarize(from, formulas)</code></td><td>Define aggregation formulas for <code>rollup</code></td></tr>
			<tr><td><code>.rollup()</code></td><td>Execute grouping and aggregation</td></tr>
			<tr><td><code>.innerJoin / .leftJoin / .rightJoin / .fullJoin</code></td><td>SQL-style joins with a condition function</td></tr>
			<tr><td><code>.crossJoin(other)</code></td><td>All combinations of two datasets</td></tr>
			<tr><td><code>.nestedJoin(other, cond)</code></td><td>Nest matching rows under a <code>children</code> key</td></tr>
			<tr><td><code>.semiJoin / .antiJoin</code></td><td>Filter by presence or absence of match in another dataset</td></tr>
	...
	<p>
		...
		<code>children</code> array per group. Add <code>summarize</code> to compute aggregate values
		instead:
	</p>
	<pre><code>{`import { mean, sum } from 'd3-array'
...
dataset(sales)
  ...
  .select()`}</code></pre>
	...
	<p>
		A key use case is animation frames — ensuring every <code>(x, color)</code> group has a data
		point for every frame value (e.g. year), filling gaps with zero so bars animate smoothly rather
		than jumping in and out.
	</p>
	<pre><code>{`// Pre-aggregated: one row per (class, drv, year). Some groups may be missing certain years.
...
const flat = complete.flatMap(row =>
  row.children.map(({ actual_flag: _, ...child }) => ({
	...
	<p>
		<code>alignBy</code> adds an <code>actual_flag</code> field to all child rows (1 = real data,
		0 = filled by template). Strip it after flattening if not needed.
	</p>

	<h2>Joins</h2>
	<p>
		Join functions are available both as chainable methods on <code>DataSet</code> and as
		standalone functions on plain arrays:
	</p>
	<pre><code>{`import { innerJoin, leftJoin, crossJoin, antiJoin, semiJoin } from '@rokkit/data'
...
const active = semiJoin(orders, activeCustomers, (o, c) => o.customerId === c.id)`}</code></pre>

	<h2>filterData — declarative filtering</h2>
	<pre><code>{`import { filterData, filterObjectArray } from '@rokkit/data'
...
const search = filterObjectArray(rows, { operator: '~*', value: /rokkit/i })`}</code></pre>
	<p>
	...
	<p>
		...
		Templates bind to <code>ds.rows</code> directly — no <code>.select()</code> needed. The
		derived result auto-recomputes whenever source data or configuration changes.
	</p>
	<pre><code>{`// dataset.svelte.js must be imported — rune compilation required
...
ds.setData(freshRows)`}</code></pre>
	...
	<h2>Performance</h2>
	<p>
		Benchmarked with Bun on Apple Silicon, n=10 000 rows, 10 iterations each.
		Key improvements after native replacement of Ramda and lazy pipeline fusion:
	</p>
	<table>
		<thead>
			<tr>
	...
	<p>
		...
		<code>Map</code>-based pass. The <code>where+apply+sortBy</code> speedup comes from lazy
		pipeline fusion: the filter is applied during the map pass so only matching rows are sorted.
	</p>
</article>
