...
**Same-element matching:** When `data-mode` and `data-style` are on the same element (`<body data-mode="dark" data-style="rokkit">`), a descendant-only selector like `[data-mode="dark"] [data-style="rokkit"]` won't match.
...
// rokkit.config.js
export default {
  palettes: {                        // Custom color palettes (extend Tailwind)
    brand: { 50: '#f0f4ff', 500: '#0f4c81', 950: '#071c30' }
  },
  colors: {                          // Semantic role → palette name mapping
    primary: 'brand',                // Use custom palette
    surface: 'zinc'                  // Use built-in Tailwind palette
  },
  skins: {                           // Named palette overrides (generates skin-* classes)
    ocean: { primary: 'sky', secondary: 'teal' }
  },
  icons: {
    app: '@rokkit/icons/app.json',   // Additional icon collections
    style: 'solid',                  // Icon variant suffix (solid|outline|duotone-outline)
    overrides: {                     // Per-icon remapping
      'folder-open': 'i-phosphor:folder-open'
    }
  },
  typography: {                      // Font family CSS custom properties
    sans: '"Inter", system-ui',
    mono: '"Fira Code", monospace',
    heading: '"Playfair Display", serif'
  },
  shape: {
    radius: 'pill'                   // 'sharp' | 'rounded' | 'pill' | { sm, md, lg, xl, full }
  },
  switcher: 'manual',               // Theme switcher mode hint
  storageKey: 'rokkit-theme'         // localStorage key for persistence
}
```
