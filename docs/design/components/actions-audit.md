...
> **Update (post-audit — migration complete):** The recommended migration has since landed. `Toolbar`, `Table`, and `TreeTable` now use `Wrapper + Navigator`, and the legacy list controller (`list-controller.svelte.js`), the legacy table controller (`table-controller.svelte.js`), the old `navigator` Svelte action (`navigator.svelte.js`), and the `Traversal` class have all been removed from `@rokkit/states` / `@rokkit/actions`. `@rokkit/states` now exports only `Wrapper` and `LazyWrapper` as navigation controllers. The sections below are the original 2026-06-03 audit, retained as a historical record — the class and action names it references no longer exist in the codebase.
...
2. Delete the old action `navigator.svelte.js`, the legacy list controller (`list-controller.svelte.js`), and the legacy table controller (`table-controller.svelte.js`). Remove the corresponding exports from `@rokkit/actions` and `@rokkit/states`.
3. Delete the unused `Traversal` class (no consumers, not exported from `@rokkit/states`).
4. Leave the rest of `@rokkit/actions` (`delegateKeyboardEvents`, `keyboard`, etc.) alone — they're independent of the list-navigation stack and serve different consumers.
...
- `packages/ui/src/components/Toolbar.svelte` — instantiated the legacy list controller, `use:navigator={{ wrapper: controller, orientation }}`, `addEventListener('action', onAction)`, manual `focusByKey`.
...
The backlog assumed there were dedicated nested-tree and tabs controller classes. There weren't. Tree navigation is `Wrapper`/`LazyWrapper` + nested `ProxyTree`; tab navigation is `Wrapper` over a flat list with `orientation: 'horizontal'`. Both already collapsed into the same surface — that was part of the ADR-003 unification.
...
Toolbar (the simpler of the two stragglers) read from the legacy list controller:
...
`Wrapper.extend(_path)` and `Wrapper.range(_path)` are currently empty methods with `// not yet implemented` comments. The 11 migrated components don't use multi-select via `Wrapper` (MultiSelect manages its own selected-values array externally). The legacy list controller *does* implement `extendSelection` + `selectRange` + `selectedKeys` properly. Before retiring it, either:
...
This is the only real blocker on the migration. Estimated cost: porting `toggleSelection`, `extendSelection`, `selectRange`, and `selectedKeys` from the legacy list controller (~60 lines, mostly direct copy with `proxyTree.lookup` swapped for `lookup`) onto `Wrapper`.
...
| Port multi-select to `Wrapper` | Low–medium. New code, but logic is straightforward and the legacy list controller is a faithful reference. | Cover with a unit spec mirroring `list-controller.spec` for multi-select. |
...
- `delegateKeyboardEvents`, `keyboard.svelte.js` — independent actions used by unrelated consumers (`TableOfContents`, etc.). They were never part of the list-navigation stack and don't need touching.
