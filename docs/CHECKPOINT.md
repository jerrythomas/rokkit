...
- `4d6bb0b2` publish-gate fix (see below).
...
`dompurify@3.4.15` with no override — what #156 §3 asked for.
...
overrides), **TypeScript 7** (deferred, path verified), and a **flaky learn e2e**
(`2026-09-14-flaky-*`).
...
Nothing published. Fixed by declaring `@vitest/spy`, and the gap is closed:
`check:build` now runs each package's declaration build inside `bun run check`.
