...
<article data-article-root>
	<p>
		...
		<code>@rokkit/states</code>), the <strong>Message</strong> component (a single notification),
		and
	...
	<div style="display:flex; flex-wrap:wrap; gap:0.5rem; margin-bottom:1rem;">
		...
		<button data-button onclick={() => push('error', { dismissible: true })}
			>Persistent (dismissible)</button
		>
		{#if lastId}
...
<Message type="success" text="Changes saved." />`}
	...
	<p>
		Add <code>dismissible</code> to show a dismiss button. Wire <code>ondismiss</code> to remove it from
		your state:
	</p>
...
{/if}`}
	...
	<p>
		...
		store. The component calls <code>ondismiss</code> after the delay. Non-dismissible toasts auto-dismiss
		after 4 s by default.
	</p>

	<Code
		content={`<!-- Standalone: calls ondismiss after 4 s -->
...
<!-- Persistent: stays until user dismisses -->
alerts.push({ type: 'error', text: 'Action required', dismissible: true })`}
		language="svelte"
	/>
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
		<div data-card>
			...
			<ul>
				...
				<li>
					<strong>dismissible</strong>: Show × button — stays until dismissed (default:
					<code>false</code>)
				</li>
				<li>
					<strong>timeout</strong>: Auto-dismiss delay in ms (default: <code>4000</code> for
					non-dismissible, <code>0</code> for dismissible)
				</li>
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
		...
		<div data-card>
			...
			<ul>
				<li>
					Mount <code>AlertList</code> once at the root — it reads the global <code>alerts</code> store
				</li>
				<li>Bottom positions stack newest-first at the screen edge</li>
				<li>
					<code>role="alert"</code> on error/warning; <code>role="status"</code> on info/success
				</li>
