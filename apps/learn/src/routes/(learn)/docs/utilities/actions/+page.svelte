...
<article data-article-root>
	<p class="text-surface-z6 mb-8 text-[1.0625rem] leading-7">
		...
		automatically on destroy. <code>@rokkit/actions</code> provides actions for keyboard navigation, gestures,
		and visual effects.
	</p>
	...
	<pre><code>&lt;div use:someAction={'{'}options} onsomeevent={'{'}handler}&gt;...&lt;/div&gt;</code
		></pre>
	...
	<p>
		...
		handle the custom events it dispatches. Events fire on <strong>keyup</strong> (not keydown). Default
	...
	<p>Additional actions for visual effects and interaction patterns:</p>
	...
	<table>
		...
		<tbody>
			<tr
				><td><code>navigable</code></td><td
					>Lightweight keyboard nav — dispatches previous/next/select/collapse/expand on keyup</td
				></tr
			>
			<tr
				><td><code>navigator</code></td><td
					>Full-featured nav paired with a Rokkit controller from @rokkit/states</td
				></tr
			>
			...
			<tr
				><td><code>swipeable</code></td><td
					>Touch/pointer swipe detection (swipeleft/swiperight/swipeup/swipedown)</td
				></tr
			>
			<tr
				><td><code>fillable</code></td><td
					>Fill container on pointer drag — dispatches fill events with progress value</td
				></tr
			>
			<tr><td><code>dismissable</code></td><td>Dismiss on click-outside or Escape key</td></tr>
			<tr
				><td><code>themable</code></td><td
					>Apply data-style attribute dynamically from the vibe store</td
				></tr
			>
			<tr
				><td><code>skinnable</code></td><td
					>Apply data-palette attribute dynamically from the vibe store</td
				></tr
			>
			<tr
				><td><code>reveal</code></td><td
					>Scroll-triggered reveal animation via IntersectionObserver</td
				></tr
			>
			...
			<tr
				><td><code>delegateKeyboardEvents</code></td><td
					>Forward keyboard events from a container to a target element</td
				></tr
			>
		</tbody>
	</table>
	...
	<ul>
		...
		<li>
			<a href="/docs/utilities/navigator">Navigator</a> — How the navigator action and controllers work
			together
		</li>
	</ul>
</article>
