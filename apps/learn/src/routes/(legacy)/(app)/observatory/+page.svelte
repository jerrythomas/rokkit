...
<div class="max-w-[1060px] mx-auto px-[48px] pt-[36px] pb-[48px]">
	...
	<div class="greeting-row flex justify-between items-baseline mb-8">
		<div class="flex flex-col gap-1">
			<span class="mono text-[10.5px] uppercase text-ink-z5 tracking-[0.06em]">{data.greeting.date}</span>
			<h1 class="font-display text-[28px] font-light text-ink-z1 m-0">{m.greeting_morning({ name: data.greeting.name })}</h1>
		</div>
		<div class="flex gap-4 items-center">
			<EnsoRing score={data.ftr.score} size={110} />
			<div class="flex flex-col gap-1 min-w-[150px]">
				<span class="mono text-[11px] text-ink-z5">{m.ftr_label()} · {data.ftr.period}</span>
				<Sparkline data={data.ftr.trend} width={150} height={36} />
				<span class="mono text-[11px] text-ink-z3">
	...
	>
		...
		<div class="flex flex-col gap-3">
			<h2 class="font-display text-[28px] font-normal text-ink-z1 m-0">{data.koan.title}</h2>
			<p class="text-[14.5px] text-ink-z3 leading-relaxed m-0">{data.koan.explanation}</p>
	...
	<div class="grid gap-[30px] mb-10" style="grid-template-columns: 1.4fr 1fr">
		<section class="insights-col">
			<h3 class="font-display text-[17px] font-normal text-ink-z1 m-0 mb-[14px]">{m.section_insights()}</h3>
			<div class="flex flex-col gap-[2px]">
				{#each data.insights as insight (insight.id)}
					<div
						...
						style="grid-template-columns: auto 1fr"
					>
						<span class="kanji text-[19px] {insight.tone === 'warn' ? 'text-warning-z5' : insight.tone === 'good' ? 'text-success-z5' : 'text-ink-z5'}">{insight.kanji}</span>
						<div class="flex flex-col gap-[2px]">
							<span class="text-[10.5px] uppercase tracking-[0.04em] text-ink-z5 font-medium">{insight.title}</span>
							<span class="text-[13.5px] text-ink-z3 leading-[1.5]">{insight.body}</span>
						</div>
					</div>
				{/each}
			</div>
		...
		<section>
			<h3 class="font-display text-[17px] font-normal text-ink-z1 m-0 mb-[14px]">{m.section_adopted()}</h3>
			<div class="flex flex-col gap-[10px]">
				{#each data.adoptedTeachings as teaching (teaching)}
					<div
						class="px-[14px] py-3 bg-surface-z1 rounded-r-md"
						style="border-left: 2px solid color-mix(in oklch, var(--color-primary-z5) 12%, transparent)"
					>
						<span class="text-[13px] text-ink-z3 leading-[1.5]">{teaching}</span>
					</div>
				{/each}
			</div>
		</section>
	...
	<section class="sessions-section">
		<h3 class="font-display text-[17px] font-normal text-ink-z1 m-0 mb-[14px]">{m.section_sessions()}</h3>
