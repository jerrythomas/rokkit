...
describe('koan store', () => {
	...
	it('submitQuery auto-selects single match', () => {
		// 'crumbs' is a keyword unique to breadcrumbs across the whole 59-tile
		// catalog (guides + components). Used here as a stable single-match
		// fixture so the auto-select branch is exercised deterministically;
		// generic queries like 'theme' now match multiple tiles after the
		// guides batch landed.
		submitQuery('crumbs')
		expect(koan.activeDemoId).toBe('breadcrumbs')
		expect(koan.visitedThisSession.has('breadcrumbs')).toBe(true)
	})
