<script lang="ts">
	...
	const customColumns = [
		...
		{ name: 'salary', label: 'Annual Salary', align: 'right' as const, formatter: (v: unknown) => `$${Number(v).toLocaleString()}` }
	]
...
<Playground
	title="Table"
	description="Data table with sortable columns and keyboard navigation."
>
	{#snippet preview()}
