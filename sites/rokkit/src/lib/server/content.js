...
export function get_index() {
	const parts = fs
		.readdirSync('content/tutorial')
		.filter(is_valid)
		.map(posixify)

	return parts.map((part) => {
		const chapters = fs
			.readdirSync(`content/tutorial/${part}`)
			.filter(is_valid)
			.map(posixify)
...
export function get_exercise(slug) {
	...
	for (let i = 0; i < exercises.length; i += 1) {
		...
		if (exercise_slug === slug) {
			...
			const chapter_meta = json(
				`content/tutorial/${part_dir}/${chapter_dir}/meta.json`
			)

			const exercise_meta_file = `content/tutorial/${part_dir}/${chapter_dir}/${exercise_dir}/meta.json`
			const exercise_meta = fs.existsSync(exercise_meta_file)
				? json(exercise_meta_file)
				: {}
			...
			if (filtered.length > 0) {
				for (const match of markdown.matchAll(/```[a-z]+\n([\s\S]+?)\n```/g)) {
					...
					if (
						...
						!content.includes('/// no-file')
					) {
						throw new Error(
							`Code block lacks a \`/// file: ...\` annotation: ${dir}/README.md`
						)
					}
				}
			}
			...
			const filenames = new Set(
				Object.keys(all_files)
					.filter(
						(filename) =>
							filename.startsWith(scope.prefix) &&
							all_files[filename].type === 'file'
					)
...
function walk(cwd, options = {}) {
	...
	function walk_dir(dir, depth) {
		...
		for (const basename of files) {
			...
			if (options.exclude?.some((exclude) => posixify(name).endsWith(exclude)))
				continue
