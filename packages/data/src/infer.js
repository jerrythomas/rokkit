 ...
 */
export function getSample(data, deepScan = true) {
	if (data.length === 0) return {}
	if (!deepScan) return data[0]
	return data.reduce(
		(acc, cur) => ({
			...acc,
...
export function deriveColumns(data, options) {
	...
	const sample = scanMode === 'fast' ? data[0] : getSample(data)
 ...
 * @param {Array<import('./types').ColumnMetadata>} columns
 * @param {Array} dataArray - The data to derive column metadata from.
 ...
 */
function deriveColumnMetadata(columns, dataArray, options) {
	if (columns.length === 0) {
