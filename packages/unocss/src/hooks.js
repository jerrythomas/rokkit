 ...
 * @property {string} [defaultMode='system'] — fallback mode: 'system' (resolve the OS preference), 'light', or 'dark'. 'auto' is accepted as a legacy alias for 'system'.
...
function buildInitScript(opts) {
	...
	// Mode defaults to 'system' (or the configured defaultMode) and is resolved to
	// light/dark by the script below: a 'system'/'auto' value from ANY source (config
	// default, persisted store, or ?mode= query) resolves the OS preference pre-paint
	// instead of writing an invalid data-mode="system".
	const dm = `'${opts.defaultMode || 'system'}'`
