...
describe('buildAreas — multi-series (with color channel)', () => {
  ...
  it('sets patternId when patterns map contains the color key', () => {
    ...
    const areas = buildAreas(data, { x: 'month', y: 'val', color: 'drv' }, xScale, yScale, colors, undefined, patterns)
    const fArea = areas.find((a) => a.key === 'f')
...
describe('buildStackedAreas', () => {
  ...
  it('returns one area per color group', () => {
    const areas = buildStackedAreas(data, { x: 'month', y: 'val', color: 'drv' }, xScale, yScale, colors)
    expect(areas).toHaveLength(2)
  })
  ...
  it('falls back to buildAreas when no color channel', () => {
    const single = [{ month: 1, val: 10 }, { month: 2, val: 20 }]
  ...
  it('sets patternId when patterns map provided', () => {
    const patterns = new Map([['f', 'diagonal'], ['4', 'dots']])
    const areas = buildStackedAreas(data, { x: 'month', y: 'val', color: 'drv' }, xScale, yScale, colors, undefined, patterns)
    const fArea = areas.find((a) => a.key === 'f')
    expect(fArea.patternId).toBe('chart-pat-f')
  })
})
