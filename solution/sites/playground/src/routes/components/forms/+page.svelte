<script lang="ts">
	...
	let selectedFlight = $state<unknown>(undefined)
	...
	let selectedHotel = $state<unknown>(undefined)
	...
	let submitStatus = $state<string | null>(null)
...
<Playground
	title="FormRenderer"
	description="Schema-driven form rendering with display components, validation, and mixed layouts."
>
	{#snippet preview()}
		<div class="flex flex-col w-full max-w-[720px] gap-4 self-start p-2">
			...
			<div class="rounded-lg border border-surface-z2 bg-surface p-4">
				...
				{:else if activeDemo === 'flights'}
					...
					{#if selectedFlight}
						<div class="mt-3 p-2 rounded bg-primary-z1 text-primary-z7 text-xs">
							Selected: {(selectedFlight as any).airline} {(selectedFlight as any).flight} — {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format((selectedFlight as any).price)}
						</div>
					{/if}
				...
				{:else if activeDemo === 'hotels'}
					...
					{#if selectedHotel}
						<div class="mt-3 p-2 rounded bg-primary-z1 text-primary-z7 text-xs">
							Selected: {(selectedHotel as any).name} — {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format((selectedHotel as any).pricePerNight)}/night
						</div>
					{/if}
	...
	{#snippet controls()}
		<div class="flex flex-col gap-2 text-xs text-surface-z6">
			...
			{:else if activeDemo === 'flights'}
				<InfoField label="Selected" value={selectedFlight ? `${(selectedFlight as any).airline} ${(selectedFlight as any).flight}` : 'None'} />
			{:else if activeDemo === 'hotels'}
				<InfoField label="Selected" value={selectedHotel ? (selectedHotel as any).name : 'None'} />
			{:else if activeDemo === 'mixed'}
