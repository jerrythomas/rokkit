...
test.describe('UploadProgress — learn page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/components/upload-progress')
	...
	test('Learn/Play toggle navigates to play page', async ({ page }) => {
		...
		await page.waitForURL('**/upload-progress/play')
		await expect(page.locator('.preview-area [data-upload-progress]')).toBeVisible()
	})
})
