...
<Select {options} bind:value placeholder="Choose a fruit..." />
\`\`\`
...
<script>
  ...
  const countries = [
    { code: 'us', name: 'United States', flag: 'i-flag:us' },
    { code: 'gb', name: 'United Kingdom', flag: 'i-flag:gb' },
    { code: 'ca', name: 'Canada',         flag: 'i-flag:ca' }
  ]
  const fields = { value: 'code', text: 'name', icon: 'flag' }

  let value = $state(null)
</script>

<Select options={countries} {fields} bind:value placeholder="Select country..." />
\`\`\`
...
\`\`\`svelte
<Select options={countries} bind:value filterable filterPlaceholder="Search countries..." />
\`\`\`
...
\`\`\`svelte
<Select {options} bind:value>
  {#snippet selectedValue(item, fields)}
    {#if item}
...
<Select {options} bind:value
  icons={{
    opened: 'i-lucide:chevron-up',
    closed: 'i-lucide:chevron-down',
    checked: 'i-lucide:check'
  }}
/>
\`\`\`
...
- [MultiSelect](/docs/components/multiselect/llms.txt) — multi-item selection
- [List](/docs/components/list/llms.txt) — inline list selection
