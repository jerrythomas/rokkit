...
<script>
  ...
  const options = [
    { value: 'overview', text: 'Overview', content: 'Overview content here...' },
    { value: 'api',      text: 'API',      content: 'API docs here...' },
    { value: 'examples', text: 'Examples', content: 'Examples here...' }
  ]
...
\`\`\`svelte
<Tabs {options} bind:value orientation="vertical" />
\`\`\`

### Bottom Tabs

\`\`\`svelte
<Tabs {options} bind:value position="after" />
\`\`\`

### Centered Tabs

\`\`\`svelte
<Tabs {options} bind:value align="center" />
\`\`\`
...
<script>
  ...
  let tabs = $state([
    { value: 't1', text: 'Tab 1', content: 'Content 1' }
  ])
  let value = $state('t1')
  let nextId = 2
  ...
  function handleAdd() {
    const id = \`t\${nextId++}\`
    tabs = [...tabs, { value: id, text: \`Tab \${nextId - 1}\`, content: '' }]
    value = id
  }

  function handleRemove(v) {
    tabs = tabs.filter(t => t.value !== v)
  }
</script>

<Tabs options={tabs} bind:value editable onadd={handleAdd} onremove={handleRemove} />
\`\`\`
...
<Tabs {options} bind:value>
  {#snippet empty()}
    <div class="p-8 text-center">No tabs available</div>
  {/snippet}
</Tabs>
\`\`\`
...
- [Toggle](/docs/components/toggle/llms.txt) — compact option switcher
- [List](/docs/components/list/llms.txt) — vertical navigable list
