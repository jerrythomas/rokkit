...
describe('Wrapper — constructor', () => {
	...
	it('exposes proxyTree accessor', () => {
		const pt = new ProxyTree(flat)
		const w = new Wrapper(pt)
		expect(w.proxyTree).toBe(pt)
	})
})
...
describe('Wrapper — onchange()', () => {
	it('fires onchange when selecting a leaf item', () => {
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onchange })
		w.lookup.get('0').expanded = true
		w.select('0-0') // Apple, value: 'apple'
		expect(onchange).toHaveBeenCalledOnce()
		const [value, proxy] = onchange.mock.calls[0]
		expect(value).toBe('apple')
		expect(proxy.label).toBe('Apple')
	})

	it('does not fire onchange when selecting a group (toggles expansion instead)', () => {
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onchange })
		w.select('0') // Fruits group — toggles expansion
		expect(onchange).not.toHaveBeenCalled()
	})

	it('deduplicates — does not fire if same value is selected twice', () => {
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onchange })
		w.lookup.get('0').expanded = true

		w.select('0-0') // Apple → fires onchange
		expect(onchange).toHaveBeenCalledOnce()

		w.select('0-0') // Apple again → same value, should not fire
		expect(onchange).toHaveBeenCalledOnce()
	})

	it('fires onchange when value changes from one leaf to another', () => {
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onchange })
		w.lookup.get('0').expanded = true
		...
		w.select('0-1') // Banana → different value
		expect(onchange).toHaveBeenCalledTimes(2)
		const [value] = onchange.mock.calls[1]
		expect(value).toBe('banana')
	})

	it('onselect still fires even when onchange is deduplicated', () => {
		const onselect = vi.fn()
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onselect, onchange })
		w.lookup.get('0').expanded = true

		w.select('0-0') // Apple → both fire
		expect(onselect).toHaveBeenCalledOnce()
		expect(onchange).toHaveBeenCalledOnce()

		w.select('0-0') // Apple again → onselect fires, onchange does not
...
describe('Wrapper — moveToValue()', () => {
	it('sets focusedKey to the item matching the given value', () => {
		const w = new Wrapper(new ProxyTree(nested))
		w.moveToValue('apple')
		expect(w.focusedKey).toBe('0-0')
	})

	it('sets focusedKey for a root-level value', () => {
		const w = new Wrapper(new ProxyTree(nested))
		w.moveToValue('veg')
		expect(w.focusedKey).toBe('1')
	})

	it('no-op when value is undefined', () => {
		const w = new Wrapper(new ProxyTree(flat))
		w.moveTo('1')
		w.moveToValue(undefined)
		expect(w.focusedKey).toBe('1') // unchanged
	})

	it('no-op when value is null', () => {
		const w = new Wrapper(new ProxyTree(flat))
		w.moveTo('1')
		w.moveToValue(null)
		expect(w.focusedKey).toBe('1') // unchanged
	...
	it('no-op when value does not match any item', () => {
		const w = new Wrapper(new ProxyTree(nested))
		w.moveTo('0')
		w.moveToValue('nonexistent')
		expect(w.focusedKey).toBe('0') // unchanged
	})

	it('syncs selectedValue so subsequent select with same value deduplicates onchange', () => {
		const onchange = vi.fn()
		const w = new Wrapper(new ProxyTree(nested), { onchange })
		w.lookup.get('0').expanded = true

		// External sync: set to 'apple' via moveToValue
		w.moveToValue('apple')
		expect(w.focusedKey).toBe('0-0')

		// Now select Apple — should NOT fire onchange (value already tracked)
		w.select('0-0')
		expect(onchange).not.toHaveBeenCalled()

		// Select Banana — different value, should fire onchange
		w.select('0-1')
		expect(onchange).toHaveBeenCalledOnce()
...
describe('Wrapper — findByText()', () => {
	it('returns key of item whose label starts with query', () => {
		const w = new Wrapper(new ProxyTree(flat))
